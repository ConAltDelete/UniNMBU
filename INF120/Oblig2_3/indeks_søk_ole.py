...
__author__ = "Ole Gauslaa"
__email__ = "olga@nmbu.no"
...
def finn_unike_ord_i_streng(streng):
    ...
    streng_uten_spesial_tegn = fjern_spesialtegn(streng)
    unike_ord = set(streng_uten_spesial_tegn.split())
...
def finn_felles_ellement_i_flere_mengder(liste_av_mengder):
    ...
    for element in liste_av_mengder:
        felles_element = felles_element.intersection(element)
...
def søk_i_indeks_med_mengde(indeks, mengde_av_søkeord):
    ...
    """
    mulige_bøker = []
    for element in mengde_av_søkeord:
        if element in indeks:
            mulige_bøker.append(indeks[element])
        else:
            mulige_bøker.append(set())
...
def klargjør_søkestreng(søkestreng):
    ...
    "Hei på deg"
    """
    klargjort_streng = finn_unike_ord_i_streng(søkestreng.lower())
...
def søk_i_indeks_med_streng(indeks, søkestreng):
    ...
    mulige_bøker = søk_i_indeks_med_mengde(indeks, mengde_av_søkeord=klargjør_søkestreng(søkestreng))
    return mulige_bøker
